<x-guest-layout>
    <div class="min-h-screen flex items-center justify-center bg-slate-950 px-4 py-12">
        <div class="w-full max-w-md">
            <div class="rounded-3xl bg-slate-900/95 border border-slate-800 p-8 shadow-2xl shadow-slate-950/50">
                ...
                <x-auth-session-status
                    class="mb-4 rounded-2xl border border-cyan-500/20 bg-slate-950/80 p-4 text-sm text-cyan-200"
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="email"
                            class="mt-2 block w-full rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3 text-white shadow-sm focus:border-cyan-500 focus:ring-cyan-500/40"
                            type="email"
                            name="email"
                            :value="old('email')"
                            required autofocus />
